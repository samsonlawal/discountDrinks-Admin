      ...
      <div className="w-full flex items-center gap-6">
         {data.map((item) => {
            ...
            return (
               ...
               >
                  <span className="text-inherit font-medium text-sm leading-[22px]">{item?.title}</span>
                  {item?.showBadge && (typeof item?.badge === 'string' || typeof item?.badge === 'number') ? (
                     <span className="text-[#4E4E4E] text-[10px] leading-4 p-[6px] rounded-[13px] bg-[#F3F3F3] ml-[6px]">
                        {item?.badge}
                     </span>
                  ) : null}
               </button>
