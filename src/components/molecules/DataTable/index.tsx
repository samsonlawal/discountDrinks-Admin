      ...
      <>
         {!showFilterColumn && !leftTopChild ? null : (
            <div className="flex w-full pb-[16px] items-end justify-center ">
               {leftTopChild ? <div className="flex-1 flex items-end justify-start">{leftTopChild}</div> : null}
         ...
         <div className=" border relative min-h-[440px] ">
            {loading ? (
                        ...
                        return (
                           <TableRow key={row.id} data-state={row.getIsSelected() && 'selected'} className="">
                              {row.getVisibleCells().map((cell) => (
                                 <TableCell key={cell.id} className="border-b border-[#e2e8f0] pl-[19px]">
